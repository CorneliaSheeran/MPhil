...
total = total[:, selection, :] #select 50 random pairs
...
coords_path = "/imaging/shared/users/ja02/CBUActors/SpatialRNNs/Results/mazeGenI_SE1_sWc_mtIV_1k/SN_539_20220404-160526.mat"
coordinates = sio.loadmat(coords_path)['Coordinates']
D = squareform(pdist(np.transpose(coordinates)))

dist_triplets = np.zeros((2500, 3))

for i in range(0, 25):
    for j in range(0, 100):

            d1 = D[total[j, i, 0], total[j, i, 1]]
            d2 = D[total[j, i, 0], total[j, i, 2]]
            d3 = D[total[j, i, 1], total[j, i, 2]]
            da_av = (d1+d2+d3)/3
            db_av = (d1+d2)/2
            dc_av = d3
            
            dist_triplets[i*j, 0] = da_av #all sides av
            dist_triplets[i*j, 1] = db_av #input-output av
            dist_triplets[i*j, 2] = dc_av #dist inputs
            

dist_triplets.tofile("/home/cs07/PIDdist_se/dist_" + str(n) + '_.csv', sep = ',')
...
data_list_post = []
data_list_pre  = []
#complete = glob.glob(dat_path + "SN_" + str(n) + "_*.mat")
#sized = 5

trial_list = glob.glob(dat_path + "SN_" + str(n) + "_*.mat")
...
pid_all = np.zeros((num_trials, normed, N, 4)) #trials, input pairs, target, PID


for i in range(num_trials):
    data_store_post = np.zeros((unit, time))
    data_store_pre = np.zeros((unit, time))

    k=0
    for j in range((i*unit), ((i+1)*unit)):
        k = k + 1
        data_store_post[k-1, :] = np.array(post_data[j])
        data_store_pre[k-1, :] = np.array(pre_data[j])
        
    for g1 in range(total.shape[0]):
        for g2 in range(total.shape[1]):
            pid_all[i, g2, g1, :] = PID.mmi(data_store_pre[total[g1, g2, 0], :], data_store_pre[total[g1, g2, 1], :], data_store_post[total[g1, g2, 2], :])


pid_all = np.reshape(pid_all, (num_trials, normed*N, 4))

pid_all.tofile("/home/cs07/PIDdist_se/se_" + str(n) + '_.csv', sep = ',')
